...
A reduction in total scanning time not only enables the user to perform more measurements during the allotted beamtime, but also reduces the radiation dose inflicted on the sample during the measurements. A reduction in radiation dose is the first step to in vivo measurements at TOMCAT. Irradiating living organisms for a prolonged time is not feasible and desired, especially if biological development is studied, which require multiple exposures of the same individual over a prolonged time-frame\todo{Do we need to elaborate more here, or is it sufficient if we come back to it in the results?}.
