...
A sequence of 19 protocols with varying quality has been scanned to assess the simulations. The 19 different scans have been chosen according to the MATLAB-plot explained in section~\ref{seq:Image Acquisition}\footnote{generated with ``MATLAB/WideFielsScanning/main.m'' for a protocol with the diameter of \SI{3.8}{mm} and three overlapping subscans}\todo{do we provide the code for reproducible research? or is it ``closed source''?}.
...
\subsubsection{Increasing the FOV}
\begin{itemize}
	\item compare ``s$_{2}$'' and ``merge'' in figures~\ref{fig:overview}~\&~\ref{fig:down}
	\item only partial acinus with s$_{2}$
	\item multiple acini with merge
	\item FOV increase is needed
	\item it works $\rightarrow$ quality-guided in next section
\end{itemize}
...
%				    % 458 px = 1.4336 mm > 100 px = 313 um > 160 px = 500 um
	...
	\label{fig:overview}%
	\todo[inline]{scale bars are not finalized, need to be adjusted to sensible lengths}
\end{figure*}
...
The 19 different protocols which have been obtained from the same sample have been three dimensionally visualized and analyzed using MeVisLab (Version 1.6.1 (2008-09-21 Release), MeVis Medical Solutions AG, Bremen, Germany). Using a region growing algorithm~\cite{wiki:regiongrowing}, airway segments have been extracted from the tomographic dataset. To be able to compare the the extracted airway segments (segments for two protocols are shown in figures~\ref{fig:BvsT}\subref{subfig:BvsToverviewB}--\subref{subfig:BvsTsegmentT}) we used exactly the same seed points for all protocols. Since the histograms of the slices from the different protocols were not completely equal, slightly different threshold values have been used to define if a voxel is include in the airway segment or not. All these threshold have been chosen in such a way that they were placed at the same location in the histogram\todo{do we need to show this, or is explanation enough?}.
...
\begin{figure*}
	...
	\subfloat[Slice 1024 of dataset for protocol B]{%
	...
	\subfloat[Slice 1024 of dataset for protocol T]{%
